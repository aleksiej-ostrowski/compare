...
# print("data = {d}\nlabels = {l}\nx = {x}\nxlabel = {xl}\nylabel = {yl}\ntitle = {t}".format(d = p.data, l = p.labels, x = p.x, xl = p.xlabel, yl = p.ylabel, t = p.title))
# exit()
...
for i, e in enumerate(p.Data):
    ...
    only_this = filter(eval("lambda x: {r}".format(r = p.Xfilter.replace('x', 'x[0]').replace('y', 'x[1]'))), for_filter)

    for a, b in only_this: 
        # b_fmt = humanize.precisedelta(dt.timedelta(milliseconds = b), minimum_unit = "milliseconds")
